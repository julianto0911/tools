...
func TestRequestWithJSON(t *testing.T) {
	...
	mockHandler := func(req *http.Request) *http.Response {
		...
		return &http.Response{
			StatusCode: 200,
			Body:       io.NopCloser(bytes.NewBufferString(lib_converter.ToJSONString(mockResp))),
			Header:     make(http.Header),
		}
	...
	assert.Equal(t, b, lib_converter.ToBytes(mockResp), "object should be same")
...
func TestMockClient(t *testing.T) {
	...
	mockHandler := func(req *http.Request) *http.Response {
		...
		return &http.Response{
			StatusCode: 200,
			Body:       io.NopCloser(bytes.NewBufferString(lib_converter.ToJSONString(""))),
			Header:     make(http.Header),
		}
	}
