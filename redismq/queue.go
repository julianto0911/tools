...
func CreateQueue(redisHost, redisPort, redisPassword string, redisDB int, name string) *Queue {
...
func SelectQueue(redisHost, redisPort, redisPassword string, redisDB int, name string) (queue *Queue, err error) {
	...
	isMember, err := redisClient.SIsMember(ctx, masterQueueKey(), name).Result()
	if err != nil {
		return nil, err
	}
	if !isMember {
...
func newQueue(redisHost, redisPort, redisPassword string, redisDB int, name string) *Queue {
	...
	q.redisClient.SAdd(ctx, masterQueueKey(), name)
	q.startStatsWriter()
	return q
}
...
func (queue *Queue) Delete() error {
	...
	for _, name := range consumers {
		...
		err = queue.redisClient.SRem(ctx, queueWorkersKey(queue.Name), name).Err()
		if err != nil {
			return err
		}
	}
	...
	err = queue.redisClient.SRem(ctx, masterQueueKey(), queue.Name).Err()
	if err != nil {
		return err
	}
	err = queue.redisClient.Del(ctx, queueWorkersKey(queue.Name)).Err()
	if err != nil {
		return err
	}
...
func (queue *Queue) Put(payload string) error {
	...
	lpush := queue.redisClient.LPush(ctx, queueInputKey(queue.Name), p.getString())
...
func (queue *Queue) RequeueFailed() error {
	...
	for l > 0 {
		err := queue.redisClient.RPopLPush(ctx, queueFailedKey(queue.Name), queueInputKey(queue.Name)).Err()
		if err != nil {
			return err
		}
		queue.incrRate(queueInputRateKey(queue.Name), 1)
...
func (queue *Queue) writeStatsCacheToRedis(now int64) {
	for sec := range queue.rateStatsCache {
		...
		for name, value := range queue.rateStatsCache[sec] {
			...
			queue.redisClient.IncrBy(ctx, key, value)
			// save stats to redis with 2h expiration
			queue.redisClient.Expire(ctx, key, 2*time.Hour)
		}
		// track queue lengths
		inputKey := fmt.Sprintf("%s::%d", queueInputSizeKey(queue.Name), now)
		failKey := fmt.Sprintf("%s::%d", queueFailedSizeKey(queue.Name), now)
		queue.redisClient.Set(ctx, inputKey, strconv.FormatInt(queue.GetInputLength(), 10), 2*time.Hour)
		queue.redisClient.Set(ctx, failKey, strconv.FormatInt(queue.GetFailedLength(), 10), 2*time.Hour)
...
func (queue *Queue) AddConsumer(name string) (c *Consumer, err error) {
	...
	added, err := queue.redisClient.SAdd(ctx, queueWorkersKey(queue.Name), name).Result()
	if err != nil {
		return nil, err
	}
	if added == 0 {
...
func (queue *Queue) isActiveConsumer(name string) bool {
	val := queue.redisClient.Get(ctx, consumerHeartbeatKey(queue.Name, name)).Val()
	return val == "ping"
}
