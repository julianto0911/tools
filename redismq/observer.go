...
func (observer *Observer) GetAllQueues() (queues []string, err error) {
	return observer.redisClient.SMembers(ctx, masterQueueKey()).Result()
}

func (observer *Observer) getConsumers(queue string) (consumers []string, err error) {
	return observer.redisClient.SMembers(ctx, queueWorkersKey(queue)).Result()
}
...
func (observer *Observer) fetchStat(keyName string, seconds int64) int64 {
	...
	vals, err := observer.redisClient.MGet(ctx, keys...).Result()
	if err != nil {
		return 0
	}
	nilVal := 0
